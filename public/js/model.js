...
export function fresh(day=today()){return{id:crypto.randomUUID(),title:'',category:'Full Time',project:'',kind:'Delivery',priority:'Medium',status:'To do',due_date:day,scheduled_date:day,start_time:'',duration:60,notes:'',top_focus:false};}
export function conflicts(t,tasks){if(!t.start_time||!t.scheduled_date||t.status==='Done')return[];return tasks.filter(x=>x.id!==t.id&&x.status!=='Done'&&x.scheduled_date===t.scheduled_date&&x.start_time&&mins(x.start_time)<mins(t.start_time)+t.duration&&mins(t.start_time)<mins(x.start_time)+x.duration)}
