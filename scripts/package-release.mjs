...
const schema=['01_schema.sql','03_pic_reports.sql','04_recurring_tasks.sql','05_telegram_workflow.sql','07_telegram_sync_pic.sql'];
...
const full=combine(sql,'Focusdesk v1.4.0 FRESH INSTALL ONLY'),upgrade=combine(sql.slice(1),'Focusdesk v1.0/v1.1/v1.2/v1.3 → v1.4.0 ADDITIVE UPGRADE');
const fullName='Focusdesk_Supabase_v1_4.sql',upgradeName='Focusdesk_Upgrade_to_v1_4.sql';
await writeFile(resolve(output,fullName),full);await writeFile(resolve(output,upgradeName),upgrade);
await copyFile('supabase/07_telegram_sync_pic.sql',resolve(output,'Focusdesk_v1_3_to_v1_4.sql'));
await copyFile('README.md',resolve(output,'Focusdesk_README_v1_4.md'));
const stream=createWriteStream(resolve(output,'Focusdesk_Vercel_v1_4.zip')),zip=archiver('zip',{zlib:{level:9}});
...
for(const f of ['dashboard-desktop.png','dashboard-mobile.png','live-status-modal.png']){
...
await zip.finalize();await done;console.log('Created Focusdesk v1.4.0 ZIP, full SQL, additive upgrade SQL and README in '+output);
zip.file('supabase/07_telegram_sync_pic.sql',{name:'Focusdesk_v1_3_to_v1_4.sql'});
