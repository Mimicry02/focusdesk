...
import {actions,actionStatus} from '../public/js/workflow.js';
...
export function keyboard(t,owner=true){return {inline_keyboard:[...actions(t,owner).map(([a,label])=>[{text:label,callback_data:`t:${t.id}:${t.version}:${a}`}]),[{text:'Add progress',callback_data:`t:${t.id}:${t.version}:note`}]]};}
...
async function delivery(o){
 ...
 try{
  ...
  if(o.kind==='task'){
   ...
   text=card(t,Object.fromEntries(rows.map(r=>[r.user_id,r.telegram_id])));markup=keyboard(t,!o.target_user_id||o.target_user_id===t.user_id);
  }else{
...
export async function receive(update){
 ...
 try{
  ...
  if(/^\/(tasks|today)(?:@[A-Za-z0-9_]+)?$/.test(text.trim())){
   ...
   for(const t of ts.slice(0,10))await sb('/rest/v1/fd_telegram_outbox?on_conflict=dedupe_key',{method:'POST',admin:true,headers:{Prefer:'resolution=ignore-duplicates'},data:{owner_id:t.user_id,target_user_id:group?null:user,target_group_id:group?.id||null,task_id:t.id,kind:'task',dedupe_key:`command:${update.update_id}:${t.id}`}});
   await respond(ts.length?`${Math.min(ts.length,10)} kartu dimasukkan antrean. Tugas lain tersedia di Focusdesk.`:'Tidak ada tugas hari ini/tertunda yang dapat Anda akses.');
  ...
  await rpc('fd_tg_action',{p_update:update.update_id,p_telegram:from.id,p_chat:chat,p_message:q?m.message_id:m.reply_to_message.message_id,p_task:t.id,p_version:input.version,p_status:status,p_note:input.note??null});
 ...
 finally{if(q)try{await telegram('answerCallbackQuery',{callback_query_id:q.id})}catch{}}
}
