...
const archive=await unzipper.Open.file(folder+'/Focusdesk_Vercel_v1_3.zip');const names=archive.files.map(x=>x.path);
for(const f of ['README.md','package.json','api/telegram.js','api/telegram-webhook.js','api/telegram-cron.js','supabase/05_telegram_workflow.sql','supabase/06_optional_queue_worker.sql','docs/PRD.md','docs/TEST_RESULTS.md','Focusdesk_Upgrade_to_v1_3.sql'])assert.ok(names.includes(f),f);
assert.ok(!names.some(x=>/(^|\/)(node_modules|dist|\.git|qa)\//.test(x)));
assert.ok(!names.includes('.env'));assert.ok(!names.includes('.env.local'));
assert.equal(JSON.parse((await archive.files.find(x=>x.path==='package.json').buffer()).toString()).version,'1.3.0');
const full=await readFile(folder+'/Focusdesk_Supabase_v1_3.sql','utf8'),upgrade=await readFile(folder+'/Focusdesk_Upgrade_to_v1_3.sql','utf8');
...
assert.equal((await archive.files.find(x=>x.path==='Focusdesk_Upgrade_to_v1_3.sql').buffer()).toString(),upgrade);
