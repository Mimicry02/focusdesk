...
import {aiConfig,checkAIConnection} from '../server/ai.js';
export default handler(async(req,res)=>{
 ...
 const [master,health]=await Promise.all([
 sb('/rest/v1/rpc/fd_desk_admin',{method:'POST',token:ctx.token,data:{p_kind:'list',p_data:{}}}),
 sb('/rest/v1/rpc/fd_desk_ai_health',{method:'POST',token:ctx.token,data:{}})
 ]);return reply(res,200,{profile:ctx.profile,master,ai:{...aiConfig(),...health}});
 }
 if(req.method!=='POST')throw new HttpError(405,'Method not allowed');
 if(ctx.profile.role!=='admin')throw new HttpError(403,'Master hanya untuk admin');
 const b=body(req,350000);
 if(b.kind==='ai-test')return reply(res,200,await checkAIConnection());
 if(!['seed','application','category','priority','route','binding','article'].includes(b.kind))throw new HttpError(400,'Operasi tidak valid');
