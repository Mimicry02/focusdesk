...
export async function sendDigest(userId){
 ...
 try{
  const rows=await sb('/rest/v1/fd_tasks?status=neq.Done&and=(or(user_id.eq.'+userId+',assignee_id.eq.'+userId+'),or(scheduled_date.eq.'+day+',due_date.lt.'+day+'))&select=title,category,start_time,duration,scheduled_date,due_date,top_focus,user_id,pic_id,pic_name,assignee_id&order=start_time.asc.nullslast&limit=500',{admin:true});
  const today=rows.filter(t=>t.scheduled_date===day),late=rows.filter(t=>t.due_date&&t.due_date<day),top=today.filter(t=>t.top_focus&&t.user_id===userId&&(!t.pic_id||t.assignee_id===userId));
  const text=`Halo ${p.display_name||'Anda'},\n\nRencana ${day} (WIB)\n\nTOP PRIORITIES\n${top.map(t=>'- '+t.title).join('\n')||'Belum dipilih'}\n\nJADWAL HARI INI\n${today.map(t=>`${t.start_time?.slice(0,5)||'Fleksibel'} · ${t.title} (${t.duration} menit) · ${t.category}${t.pic_id?' · PIC: '+t.pic_name:''}${t.user_id!==userId?' · ditugaskan kepada Anda':''}`).join('\n')||'Belum ada tugas terjadwal'}\n\nTERLAMBAT\n${late.map(t=>'- '+t.title+' · deadline '+t.due_date).join('\n')||'Tidak ada'}\n\nBuka workspace: ${process.env.APP_URL}\nMatikan briefing melalui Settings di aplikasi.\n${rows.length===500?'Daftar dibatasi 500 tugas. Buka aplikasi untuk semua tugas.':''}`;
