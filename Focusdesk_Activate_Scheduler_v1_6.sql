-- ACTIVATE AFTER deploying v1.6 and running migration 08.
...
do $$begin
 ...
 if exists(select 1 from vault.decrypted_secrets where name in ('focusdesk_app_url','focusdesk_cron_secret') and decrypted_secret ~* '(ISI_DENGAN|TEMPEL_|DOMAIN-FOCUSDESK|your-app|focusdesk-anda)') then raise exception 'Vault masih berisi placeholder. Isi domain dan secret asli.';end if;
